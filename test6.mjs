import fs from 'fs';
const url = 'https://rmnceswyiscbnfzymjci.supabase.co/rest/v1/receitas?select=' + encodeURIComponent('*, receita_ingredientes!receita_id(*, receitas!ingrediente_receita_id(*))');
...
async function test(url) {
    ...
    fs.writeFileSync('success.json', JSON.stringify(await res.json(), null, 2));
}
test(url);
